      ...
      def self.included(base)
        base.class_eval do
          ...
          def read
            ...
            self.popular_posts = self.posts.docs.sort do |post_x, post_y|
              if  defined? post_y.page_rank() && post_y.page_rank(host) > post_x.page_rank(host) then 1
              elsif  defined? post_y.page_rank() &&  post_y.page_rank(host) == post_x.page_rank(host) then 0
              else -1
              end
            end
          end
