...
The device runs a node.js app that handles configuration and also posting data to a web service running in the cloud. Each user has an account and can have 1 or more devices. I won't get into the purpose of the device here (or a few more components it has) but the device is working for its intended early prototyping purpose. I have a drawer full of SparkFun Edison blocks. They are rather expensive but so quick to use for rapid prototyping simple devices, and integrate well with various node libs like <a href="https://www.npmjs.com/package/cylon" target="_blank">cylon</a>and <a href="https://github.com/node-red/node-red" target="_blank">nodered</a>. But when it is time to build more than a few prototypes, cost does start to matter.  One issue is the Edison platform is too expensive to create devices at the price point I am targetting for this latest device.
